import { HttpErrorResponse } from '@angular/common/http';
import { Component, EventEmitter, Input, Output } from '@angular/core';
import { FormBuilder, ReactiveFormsModule, Validators } from '@angular/forms';
import { finalize } from 'rxjs';
import { ToastrService } from 'ngx-toastr';

import { AdminProductService } from '../../core/services/admin-products.service';

@Component({
  selector: 'app-bulk-reject-modal',
  standalone: true,
  imports: [ReactiveFormsModule],
  templateUrl: './bulk-reject-modal.component.html',
  styleUrl: './bulk-reject-modal.component.css'
})
export class BulkRejectModalComponent {
  @Input() productIds: number[] = [];

  @Output() closed = new EventEmitter<void>();
  /** Emits the ids that were rejected, so the parent can remove them from its list. */
  @Output() rejected = new EventEmitter<number[]>();

  readonly form = this.fb.nonNullable.group({
    reason: ['', [Validators.required, Validators.maxLength(2000)]]
  });

  submitting = false;

  get count(): number {
    return this.productIds.length;
  }

  constructor(
    private readonly fb: FormBuilder,
    private readonly adminProductService: AdminProductService,
    private readonly toastr: ToastrService
  ) { }

  onBackdropClick(): void {
    this.dismiss();
  }

  dismiss(): void {
    if (this.submitting) return;
    this.resetForm();
    this.closed.emit();
  }

  confirm(): void {
    if (this.count === 0 || this.submitting) return;

    if (this.form.invalid) {
      this.form.markAllAsTouched();
      return;
    }

    const reason = this.form.controls.reason.value.trim();
    if (!reason) {
      this.form.controls.reason.setErrors({ required: true });
      this.form.controls.reason.markAsTouched();
      return;
    }

    this.submitting = true;

    this.adminProductService
      .bulkReject({ productIds: this.productIds, reason })
      .pipe(finalize(() => (this.submitting = false)))
      .subscribe({
        next: (res) => {
          if (res?.success) {
            this.toastr.success(`${this.count} product(s) rejected.`, 'Rejected');
            const ids = [...this.productIds];
            this.resetForm();
            this.rejected.emit(ids);
            this.closed.emit();
          } else {
            this.toastr.error(res?.message?.trim() || 'Could not reject the selected products.', 'Error');
          }
        },
        error: (err: unknown) => {
          this.toastr.error(this.messageFromHttp(err), 'Error');
        }
      });
  }

  private resetForm(): void {
    this.form.reset({ reason: '' });
  }

  private messageFromHttp(err: unknown): string {
    if (err instanceof HttpErrorResponse) {
      const body = err.error;
      if (typeof body === 'object' && body && 'message' in body) {
        return String((body as { message: string }).message);
      }
      if (typeof body === 'string' && body.trim()) return body;
      if (err.status === 403) return 'You are not allowed to reject products.';
    }
    return 'Could not reject the selected products. Please try again.';
  }
}
